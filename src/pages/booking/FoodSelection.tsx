...
const FoodSelection = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [movieRes, foodRes] = await Promise.all([
          movieId ? movieApi.getMovieById(movieId) : Promise.resolve({ data: null }),
          foodApi.getFoods()
        ]);
  ...
  const calculateTotal = () => {
    return foods.reduce((acc, item) => {
      return acc + ((item.price || 0) * (cart[item._id || item.id] || 0));
    }, 0);
  };
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-sm text-primary font-bold uppercase tracking-widest mt-1 flex items-center gap-2">
              Enhance Your Experience <span className="w-1.5 h-1.5 bg-primary/40 rounded-full" /> {movie?.title || "Unknown Movie"}
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-12">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {foods.map((item) => {
                ...
                // Add fallback image if it's not present from DB
                const itemImage = item.image || (item.type === 'COMBO' 
                  ? 'https://images.unsplash.com/photo-1594465919760-441fe5908ab0?auto=format&fit=crop&q=80&w=200'
                  : 'https://images.unsplash.com/photo-1578345218746-50a229b3d0f8?auto=format&fit=crop&q=80&w=200');

                return (
                  <div key={itemId} className="glass-card rounded-[2.5rem] p-8 flex gap-8 items-center group hover:border-primary/30 transition-all shadow-inner-glossy">
                    <div className="w-32 h-32 rounded-3xl overflow-hidden shadow-2xl shrink-0 border border-white/5">
                      <img src={itemImage} alt={item.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                    </div>
                    <div className="flex-1 space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                         <span className="text-xl font-black text-primary">${(item.price || 0).toFixed(2)}</span>
                         <div className="flex items-center gap-4 bg-background/50 rounded-2xl p-1 border border-white/5 shadow-inner-glossy">
                            ...
                            <span className="text-sm font-black text-white w-6 text-center">{cart[itemId] || 0}</span>
                            <button 
                              onClick={() => updateQuantity(itemId, 1)}
                 ...
                 {foodTotal > 0 && (
                   <div className="space-y-4 animate-in slide-in-from-top-4 duration-500">
                      ...
                      <div className="space-y-3">
                         {foods.filter(item => cart[item._id || item.id] > 0).map(item => {
                           const itemId = item._id || item.id;
                           return (
                             <div key={itemId} className="flex justify-between items-center text-sm font-bold text-white/80">
                                <span>{cart[itemId]} x {item.name}</span>
                                <span>${((item.price || 0) * cart[itemId]).toFixed(2)}</span>
                             </div>
                           )
                         })}
                      </div>
